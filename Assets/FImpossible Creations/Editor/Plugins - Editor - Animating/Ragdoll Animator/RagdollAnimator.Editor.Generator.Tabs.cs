...
namespace FIMSpace.FProceduralAnimation
{
    ...
    public partial class RagdollAnimatorEditor
    {
                    ...
                    else // Custom Limbs Menu
                    {
                        ...
                        if (proc.CustomLimbsBonesChains.Count > 0)
                        {
                            ...
                            if (_selectedCustomChain > -1)
                            {
                                ...
                                GUILayout.Space(2);
                                chain.UnlimitedRotations = EditorGUILayout.Toggle(new GUIContent("Unlimited Rotations:", "Bypassing configurable joint limits. Can be useful for tails/tentacles."), chain.UnlimitedRotations);
                                ...
                                if (_chainModeDraw == EBonesChainDrawMode.BonesSetup)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        var optionButton = lastRect;
                                        optionButton.width = 22;
                                        optionButton.x += lastRect.width - 21;
                                        optionButton.y += 10;
                                        optionButton.height = 16;

                                        if (GUI.Button(optionButton, FGUI_Resources.GUIC_More, EditorStyles.label))
                                        {
                                            GenericMenu menu = new GenericMenu();

                                            menu.AddItem(new GUIContent("Copy Bones Settings"), false, () => { RagdollProcessor.BonesChain.CopyFrom = chain; });
                                            if (RagdollProcessor.BonesChain.CopyFrom != null)
                                            {
                                                menu.AddSeparator("");

                                                string copyFromName = RagdollProcessor.BonesChain.CopyFrom.ChainName;
                                                menu.AddItem(new GUIContent("Paste '" + copyFromName + "' Bones Settings"), false, () =>
                                                {
                                                    RagdollProcessor.BonesChain.PasteMainSettingsTo(chain);
                                                    EditorUtility.SetDirty(Get);
                                                    if (generator != null) if (generator.IsGenerateEnabled) generator.forceUpdateGenerate = true;
                                                });

                                                menu.AddItem(new GUIContent("Paste '" + copyFromName + "' Bones Settings (for symmetry)"), false, () =>
                                                {
                                                    RagdollProcessor.BonesChain.PasteMainSettingsTo(chain, true);
                                                    EditorUtility.SetDirty(Get);
                                                    if (generator != null) if (generator.IsGenerateEnabled) generator.forceUpdateGenerate = true;
                                                });
                                            }
                                        ...
                                        EditorGUI.BeginChangeCheck();
                                        chain.ChainScaleMultiplier = EditorGUILayout.Slider(new GUIContent("Chain Scale Multiplier:", "Increasing scale of all chain's bones colliders."), chain.ChainScaleMultiplier, 0.1f, 2f, GUILayout.Width(EditorGUIUtility.currentViewWidth - 74));
                                        if (EditorGUI.EndChangeCheck()) if (generator != null) if (generator.IsGenerateEnabled) generator.forceUpdateGenerate = true;
                                        EditorGUIUtility.labelWidth = 168;
                                        EditorGUI.BeginChangeCheck();
                                        chain.AxisLimitRange = EditorGUILayout.Slider(new GUIContent("Joints Rotation Limit Range:", "Limiting joints rotation angle for all bones in chain: 1 = 90 degrees : 0.01 = 1 degrees."), chain.AxisLimitRange, 0.01f, 1f);
                                        EditorGUIUtility.labelWidth = 0;
                                        if (EditorGUI.EndChangeCheck()) if (Application.isPlaying) chain.RefreshJointsLimitsPlaymode();
                                        ...
                                        if (dropEvent != null)
                                            if (dropEvent.type == EventType.DragPerform || dropEvent.type == EventType.DragUpdated)
                                                if (dropRect.Contains(dropEvent.mousePosition))
                                                {
                                                    ...
                                                    if (dropEvent.type == EventType.DragPerform)
                                                    {
                                                        ...
                                                        foreach (var dragged in DragAndDrop.objectReferences)
                                                        {
                                                            if (dragged is GameObject)
                                                            {
                                                                ...
                                                                { if (chain.BoneSetups[i].t == go.transform) { addIt = false; break; } }

                                                                if (addIt)
                                                                {
                                                                    var bone = new RagdollProcessor.RagdollBoneSetup();
                                                                    bone.t = go.transform;
                                                                    chain.BoneSetups.Add(bone);
                                                                    SceneView.RepaintAll();
                                                                }
                                                            }
                                                        }
        ...
        private void DrawBoneSetupGUI(bool extraValues = false, RagdollProcessor.BonesChain chain = null)
        {
            ...
            if (_selectedBoneSetup != null)
            {
                ...
                if (_selectedBoneSetup != null)
                {
                    ...
                    if (_customChain_limitsFoldout) GUI.backgroundColor = Color.green;
                    if (GUILayout.Button(new GUIContent(FGUI_Resources.Tex_Limits, "Switch displaying joint rotation limits.\nRead unity docs for explanation of joint limits."), FGUI_Resources.ButtonStyle, GUILayout.Height(18), GUILayout.Width(22))) _customChain_limitsFoldout = !_customChain_limitsFoldout;
                    GUI.backgroundColor = Color.white;

                    if (_customChain_limitsFoldout)
                    {
                        EditorGUI.BeginChangeCheck();
                        ...
                        if (EditorGUIUtility.currentViewWidth > 440)
                        {
                            ...
                            _selectedBoneSetup.LowTwistAngleLimit = EditorGUILayout.FloatField("Low Twist:", _selectedBoneSetup.LowTwistAngleLimit);
                            _selectedBoneSetup.LowTwistAngleLimit = Mathf.Clamp(_selectedBoneSetup.LowTwistAngleLimit, -177f, _selectedBoneSetup.HighTwistAngleLimit);

                            _selectedBoneSetup.HighTwistAngleLimit = EditorGUILayout.FloatField("High Twist:", _selectedBoneSetup.HighTwistAngleLimit);
                            _selectedBoneSetup.HighTwistAngleLimit = Mathf.Clamp(_selectedBoneSetup.HighTwistAngleLimit, _selectedBoneSetup.LowTwistAngleLimit, 177f);


                            _selectedBoneSetup.LowSwingAngleLimit = EditorGUILayout.FloatField("Low Swing:", _selectedBoneSetup.LowSwingAngleLimit);
                            _selectedBoneSetup.LowSwingAngleLimit = Mathf.Clamp(_selectedBoneSetup.LowSwingAngleLimit, 3f, 177f);

                            _selectedBoneSetup.HighSwingAngleLimit = EditorGUILayout.FloatField("High Swing:", _selectedBoneSetup.HighSwingAngleLimit);
                            _selectedBoneSetup.HighSwingAngleLimit = Mathf.Clamp(_selectedBoneSetup.HighSwingAngleLimit, 3f, 177f);
                        }
                        else
                        {
                            EditorGUIUtility.labelWidth = 40;
                            EditorGUIUtility.fieldWidth = 20;
                            _selectedBoneSetup.LowTwistAngleLimit = EditorGUILayout.FloatField(new GUIContent("LTws:", "Low Twist"), _selectedBoneSetup.LowTwistAngleLimit);
                            _selectedBoneSetup.LowTwistAngleLimit = Mathf.Clamp(_selectedBoneSetup.LowTwistAngleLimit, -177f, _selectedBoneSetup.HighTwistAngleLimit);

                            _selectedBoneSetup.HighTwistAngleLimit = EditorGUILayout.FloatField(new GUIContent("HTws:", "High Twist"), _selectedBoneSetup.HighTwistAngleLimit);
                            _selectedBoneSetup.HighTwistAngleLimit = Mathf.Clamp(_selectedBoneSetup.HighTwistAngleLimit, _selectedBoneSetup.LowTwistAngleLimit, 177f);

                            _selectedBoneSetup.LowSwingAngleLimit = EditorGUILayout.FloatField(new GUIContent("LSwn:", "Low Swing"), _selectedBoneSetup.LowSwingAngleLimit);
                            _selectedBoneSetup.LowSwingAngleLimit = Mathf.Clamp(_selectedBoneSetup.LowSwingAngleLimit, 3f, 177f);

                            _selectedBoneSetup.HighSwingAngleLimit = EditorGUILayout.FloatField(new GUIContent("HSwn:", "High Swing"), _selectedBoneSetup.HighSwingAngleLimit);
                            _selectedBoneSetup.HighSwingAngleLimit = Mathf.Clamp(_selectedBoneSetup.HighSwingAngleLimit, 3f, 177f);
                        }

                        if (EditorGUI.EndChangeCheck())
                        {
                            if (Application.isPlaying) _selectedBoneSetup.ApplyJointLimitsPlaymode(chain.AxisLimitRange);
                        }
                    ...
                    {
                        EditorGUILayout.LabelField(" Hit button to display join rotation limits", EditorStyles.centeredGreyMiniLabel);
                    }
                    ...
                    if (_selectedBoneSetup.ColliderType != RagdollProcessor.RagdollBoneSetup.EColliderType.Mesh)
                    {
                        ...
                        {
                            _selectedBoneSetup.Generator_ScaleMul = EditorGUILayout.Slider(title, _selectedBoneSetup.Generator_ScaleMul, 0f, 4.001f);
                            if (_selectedBoneSetup.Generator_ScaleMul > 4f) _selectedBoneSetup.Generator_Extra = true;
                        }
                        else
        ...
        static bool _customChain_limitsFoldout = false;
