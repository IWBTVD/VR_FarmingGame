...
namespace FIMSpace.FProceduralAnimation
{
    ...
    public partial class RagdollProcessor
    {
        ...
        public class BonesChain
        {
            ...
            internal void InitializeForDummy(RagdollProcessor ragdollProcessor, Transform dummyRoot)
            {
                for (int b = 0; b < BoneSetups.Count; b++)
                {
                    ...
                    bone.Posing.UnlimitedRotationJoint = UnlimitedRotations;
                }
            ...
            /// <summary> Editor Utility but can be used for the playmode too </summary>
            public static BonesChain CopyFrom { get; set; }

            public static void PasteMainSettingsTo(BonesChain to, bool symmetry = false)
            {
                if (CopyFrom == null) return;
                PasteMainSettingsTo(to, CopyFrom, symmetry);
            }

            public static void PasteMainSettingsTo(BonesChain to, BonesChain from, bool symmetry)
            {
                PasteColliderSettingsTo(to, from, symmetry);
                PastePhysicsSettingsTo(to, from);
            }

            public static void PasteColliderSettingsTo(BonesChain to, BonesChain from, bool symmetry)
            {
                if (to == from) return;

                for (int b = 0; b < to.BoneSetups.Count; b++)
                {
                    if (b > from.BoneSetups.Count) break;
                    var toBone = to.BoneSetups[b];
                    var fromBone = from.BoneSetups[b];

                    toBone.ColliderMesh = fromBone.ColliderMesh;
                    toBone.ColliderType = fromBone.ColliderType;
                    toBone.Generator_OverrideCapsuleDir = fromBone.Generator_OverrideCapsuleDir;
                    toBone.Generator_ScaleMul = fromBone.Generator_ScaleMul;
                    toBone.Generator_LengthMul = fromBone.Generator_LengthMul;
                    toBone.Generator_LengthOffset = fromBone.Generator_LengthOffset * (symmetry ? -1f : 1f);
                    toBone.Generator_BoxScale = fromBone.Generator_BoxScale;
                    toBone.Generator_Offset = fromBone.Generator_Offset * (symmetry ? -1f : 1f);
                }
            }

            public static void PastePhysicsSettingsTo(BonesChain to, BonesChain from)
            {
                if (to == from) return;

                to.AxisLimitRange = from.AxisLimitRange;
                ...
                for (int b = 0; b < to.BoneSetups.Count; b++)
                {
                    ...
                    toBone.MassPercentage = fromBone.MassPercentage;
                    toBone.MuscleForceMultiplier = fromBone.MuscleForceMultiplier;
                    toBone.LowTwistAngleLimit = fromBone.LowTwistAngleLimit;
                    toBone.HighTwistAngleLimit = fromBone.HighTwistAngleLimit;
                    toBone.LowSwingAngleLimit = fromBone.LowSwingAngleLimit;
                    toBone.HighSwingAngleLimit = fromBone.HighSwingAngleLimit;
                }
            }
            ...
            public void RefreshJointsLimitsPlaymode()
            {
                ...
                {
                    var bone = BoneSetups[b];
                    bone.ApplyJointLimitsPlaymode(AxisLimitRange);
                }
            }
        }
        ...
        public class RagdollBoneSetup
        {
            ...
            /// <summary> Low Twist or lowAngularXLimit : -177 : 177 : Needs to be lower than High Twist Limit </summary>
            public float LowTwistAngleLimit = -60f;
            /// <summary> HighTwist or highAngularXLimit : -177 : 177 </summary>
            public float HighTwistAngleLimit = 60f;
            /// <summary> LowSwing or angularYLimit : 3 : 177</summary>
            public float LowSwingAngleLimit = 20f;
            /// <summary> HighSwing or angularZLimit : 3 : 177 </summary>
            public float HighSwingAngleLimit = 20f;
            ...
            public void ApplyJointLimitsPlaymode(float multiplier = 1f)
            {
                ...
                if ( Posing.ConfigurableJoint == null ) return;

                SoftJointLimit lim = Posing.ConfigurableJoint.highAngularXLimit;
                lim.limit = HighTwistAngleLimit * multiplier;
                Posing.ConfigurableJoint.highAngularXLimit = lim;

                lim = Posing.ConfigurableJoint.lowAngularXLimit;
                lim.limit = LowTwistAngleLimit * multiplier;
                Posing.ConfigurableJoint.lowAngularXLimit = lim;

                lim = Posing.ConfigurableJoint.angularYLimit;
                lim.limit = LowSwingAngleLimit * multiplier;
                Posing.ConfigurableJoint.angularYLimit = lim;

                lim = Posing.ConfigurableJoint.angularZLimit;
                lim.limit = HighSwingAngleLimit * multiplier;
                Posing.ConfigurableJoint.angularZLimit = lim;
            }
        }
        ...
        internal void Initialize(MonoBehaviour caller, Transform objectWithAnimator, Transform customRagdollAnimator = null, Transform rootBone = null, Transform containerForDummy = null)
        {
            ...
            {
                Animator anim = caller.transform.GetComponentInChildren<Animator>();
                if (anim) objectWithAnimator = anim.transform;
                else objectWithAnimator = caller.transform;
                UnityEngine.Debug.Log("[Ragdoll Animator] Not Assigned Object With Animator! Trying to set it automatically! (" + caller.transform.name + ")");
                ObjectWithAnimator = objectWithAnimator;
            }

            if (Pelvis == null)
            {
                Animator anim = ObjectWithAnimator.GetComponentInChildren<Animator>();
                if (anim.isHuman) Pelvis = anim.GetBoneTransform(HumanBodyBones.Hips);
                else UnityEngine.Debug.Log("[Ragdoll Animator] Not Assigned Pelvis Bone! (" + caller.name + ")");
            }
            ...
            _previousPelvisFixedPosition = Pelvis.position;
            _pelvisVelocityReference = Vector3.zero;
            _initialPelvisToHeadDistanceSqrt = 0.2f;
            if (Head != null) _initialPelvisToHeadDistanceSqrt = Vector3.SqrMagnitude(Pelvis.position - Head.position);
        }
        ...
        void Initialization(MonoBehaviour caller, Transform objectWithAnimator, Transform customRagdollAnimator = null, Transform rootBone = null)
        {
            ...
            if ( CustomLimbsBonesChains != null && CustomLimbsBonesChains.Count > 0)
            {
                for (int ch = 0; ch < CustomLimbsBonesChains.Count; ch++) CustomLimbsBonesChains[ch].RefreshJointsLimitsPlaymode();
            }
        }
        ...
        public void User_BlendOnCollisionSetCulldown_Spine(float culldown) { spineCulldown = culldown; }
        float chestCulldown = 0f;
        public void User_BlendOnCollisionSetCulldown_Chest(float culldown) { chestCulldown = culldown; }
        float minChestAnim = 0f;
        float sd_minChestAnim = 0f;
        float minSpineAnim = 0f;
        float sd_minSpineAnim = 0f;
        float larmCulldown = 0f;
        public void User_BlendOnCollisionSetCulldown_LeftArm(float culldown) { larmCulldown = culldown; }
        float rarmCulldown = 0f;
        public void User_BlendOnCollisionSetCulldown_RightArm(float culldown) { rarmCulldown = culldown; }
        float minRArmsAnim = 0f;
        float lminArmsAnim = 0f;
        float sd_lminArmsAnim = 0f;
        float sd_minRArmsAnim = 0f;
        float headCulldown = 0f;
        public void User_BlendOnCollisionSetCulldown_Head(float culldown) { headCulldown = culldown; }
        ...
        [NonSerialized] public float constantLeftArmRagdoll = 0f;
        [NonSerialized] public float constantRightArmRagdoll = 0f;

        internal void LateUpdate(bool captureAnimation = true)
        {
            if (Initialized == false) return;

            if (canDisableCalculations)
            {
                return;
            }
            ...
            if (BonesSetupMode == EBonesSetupMode.HumanoidLimbs)
            {
                ...
                float constL = Mathf.Max(ConstantArmsRagdoll, constantLeftArmRagdoll);
                float constR = Mathf.Max(ConstantArmsRagdoll, constantRightArmRagdoll);
                ...
                if (BlendOnCollision)
                {
                    ...
                    AnimateMinBlend(ref minRArmsAnim, ref sd_minRArmsAnim, rarmCulldown > 0f ? 1f : constR);
                    AnimateMinBlend(ref lminArmsAnim, ref sd_lminArmsAnim, larmCulldown > 0f ? 1f : constL);
                }
                else
                {
                    lminArmsAnim = constL;
        ...
        internal void Update()
        {
            ...
            if (canDisableCalculations) return;
        ...
        private void Calibration()
        {
            ...
            c.visibleBone.localPosition = c.initialLocalPosition;
        ...
        int fixedFrames = 0;
        float delta = 0.1f;
        bool canDisableCalculations = false;
        ...
        internal void FixedUpdate() // Sync with physics --------------------------------------
        {
            ...
            #region Optimize Calculations switch

            if (AllowDisableCalculationsOnZeroBlend)
            {
                if (finalBlend <= 0f && !BlendOnCollision)
                {
                    if (FreeFallRagdoll == false)
                    {
                        ...
                        // Hard sync dummy with the skeleton
                        posingPelvis.transform.position = posingPelvis.visibleBone.position;
                        posingPelvis.transform.rotation = posingPelvis.visibleBone.rotation;

                        PosingBone rBone = posingPelvis.child;
                        while (rBone != null)
                        {
                            rBone.transform.localRotation = rBone.visibleBone.localRotation;
                            rBone = rBone.child;
                        }

                        if (!canDisableCalculations) // On Change
                        {
                            rBone = posingPelvis.child;
                            while (rBone != null)
                            {
                                if (rBone.rigidbody != null) rBone.rigidbody.isKinematic = true;
                                rBone = rBone.child;
                            }
                        }
                    }

                    canDisableCalculations = true;
                    return;
                }
                else
                {
                    if (canDisableCalculations) // On Change
                    {
                        ...
                        while (rBone != null)
                        {
                            if (rBone.rigidbody != null) rBone.rigidbody.isKinematic = false;
                            rBone = rBone.child;
                        }
                    }

                    canDisableCalculations = false;
                }
            }
            ...
            Vector3 avgPelvisVelo = posingPelvis.rigidbody.position - _previousPelvisFixedPosition;

            if (FreeFallRagdoll)
                _pelvisVelocityReference = posingPelvis.rigidbody.velocity;
            else
                if (avgPelvisVelo.sqrMagnitude > _initialPelvisToHeadDistanceSqrt * 0.0001) _pelvisVelocityReference = avgPelvisVelo / Time.fixedDeltaTime;

            _previousPelvisFixedPosition = posingPelvis.rigidbody.position;
        ...
        void UpdateReposing()
        {
            ...
            Vector3 basePos = RagdollDummyBase.position;
            ...
            RagdollDummyBase.position = basePos;
        }
