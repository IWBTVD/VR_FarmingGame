...
namespace FIMSpace.FProceduralAnimation
{
    ...
    public class RagdollGeneratorParameters
    {
        ...
        public CollisionDetectionMode DetectionMode = CollisionDetectionMode.Discrete;

        public bool generateFists = false;
        public bool generateFoots = false;
        public bool generateShoulders = false;

        public bool AlignWithScale = false;
        ...
        public bool Editor_DrawParameters(UnityEngine.Object forUndo)
        {
            ...
            DetectionMode = (CollisionDetectionMode)EditorGUILayout.EnumPopup("Collisions Detection:", DetectionMode);
            EditorGUIUtility.labelWidth = 0;
            AlignWithScale = EditorGUILayout.Toggle(new GUIContent("Align Scale", "If you experience generated colliders being scaled like by 1000, try enabling this toggle and generate ragdoll again"), AlignWithScale);
    ...
    public class RagdollGeneratorPlayer
    {
        ...
        static bool alignWithScale = false;
        ...
        {
            alignWithScale = par.AlignWithScale;
            ...
            CollisionDetectionMode collisionDetection = par.DetectionMode;
            ...
            if (proc.BonesSetupMode == RagdollProcessor.EBonesSetupMode.CustomLimbs)
            {
                ...
                SetupPelvisBone(proc, bounciness, damper, drag, aDrag, totalMass, projection, projAngle, projDistance, enCollision, preProcessing, interpolation, collisionDetection);
                ...
                Ragdoll_AdjustCollider(bone, pelvisToNonLegLocal * 0.4f, pelvisToNonLegLocal, pelvisToNonLegLocal.magnitude * 0.35f * scale, pelvisToNonLegLocal.magnitude * 0.75f * scale, interpolation, collisionDetection, true);
                ...
                for (int i = 1; i < BoneChains.Count; i++)
                {
                    ...
                    if (chainParent)
                    {
                        float scaleMultiply = chain.ChainScaleMultiplier;
                        if (scaleMultiply <= 0f) scaleMultiply = 1f;
                        if (chain.AxisLimitRange <= 0f || chain.AxisLimitRange > 10f) chain.AxisLimitRange = 1f;
                        ...
                        for (int b = 0; b < chain.BoneSetups.Count; b++)
                        {
                            ...
                            float sScale = scale * scaleMultiply;
                            Ragdoll_AdjustCollider(bone, targetDirLocal * 0.5f, targetDirLocal * sScale, targetLength * 1f * sScale, targetLength * 0.125f * sScale, interpolation, collisionDetection);
                        }
                        ...
                        for (int b = 0; b < chain.BoneSetups.Count; b++)
                        {
                            ...
                            if (bone.LowSwingAngleLimit == 0f && bone.HighSwingAngleLimit == 0f && bone.LowTwistAngleLimit == 0f && bone.HighTwistAngleLimit == 0f) { bone.LowSwingAngleLimit = 90f; bone.HighSwingAngleLimit = 90f; bone.LowTwistAngleLimit = 90f; bone.HighTwistAngleLimit = 90f; }

                            Ragdoll_Joint(bone.t, chain.AxisLimitRange * bone.LowTwistAngleLimit, chain.AxisLimitRange * bone.HighTwistAngleLimit, chain.AxisLimitRange * bone.LowSwingAngleLimit, chain.AxisLimitRange * bone.HighSwingAngleLimit, spring, damper);
                            Ragdoll_ComputeAxis(baseTransform, bone.t, ref f, ref r, ref u);
                            Ragdoll_JointAxis(bone.t, r, null, r, f, u);
                        }
                    }

                    chain.RefreshJointsLimitsPlaymode();
                }
            }
            else
            {
                ...
                // Pelvis to head
                SetupPelvisBone(proc, bounciness, damper, drag, aDrag, massToDistr, projection, projAngle, projDistance, enCollision, preProcessing, interpolation, collisionDetection);

                t = PelvisBone;
                Vector3 pelvisToNonLegLocal = t.InverseTransformVector(chst.position - t.position);
                Ragdoll_AdjustCollider(proc.GetPelvisSetupBone(), pelvisToNonLegLocal * 0.4f, pelvisToNonLegLocal, pelvisToNonLegLocal.magnitude * 0.35f * scale, pelvisToNonLegLocal.magnitude * 0.75f * scale, interpolation, collisionDetection);
                ...
                foreach (Transform tb in bones)
                {
                    ...
                    if (rig)
                    {
                        ...
                        rig.collisionDetectionMode = collisionDetection;
                    }
                }
        ...
        void SetupPelvisBone(RagdollProcessor proc, float bounciness = 0f, float damper = 0f, float drag = 0.5f, float aDrag = 1f, float massToDistr = 65f, bool projection = true, float projAngle = 90, float projDistance = 0.05f, bool enCollision = false, bool preProcessing = false, RigidbodyInterpolation interpolation = RigidbodyInterpolation.Interpolate, CollisionDetectionMode collDetection = CollisionDetectionMode.Discrete)
        {
            ...
            PelvisBone.GetComponent<Rigidbody>().collisionDetectionMode = collDetection;
        }
        ...
        public static void Ragdoll_AdjustCollider(RagdollProcessor.RagdollBoneSetup bone, Vector3 center, Vector3 localScale, float length, float radius, RigidbodyInterpolation interpolation, CollisionDetectionMode collDetection, bool debug = false)
        {
            ...
            if (radius < 0.000051f) radius = 0.1f;
            if (length < 0.000051f) length = 0.1f;
            ...
            {
                rig.interpolation = interpolation;
                rig.collisionDetectionMode = collDetection;
            }
