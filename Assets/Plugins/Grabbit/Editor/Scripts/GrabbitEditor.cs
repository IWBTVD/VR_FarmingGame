...
namespace Grabbit
{
    public class GrabbitEditor : EditorWindow
    {
        ...
        private static void StartGrabbitOnKeyDown(GrabbitMode mode = GrabbitMode.PLACE)
        {
            if (!instance)
            {
                FetchSettings(out var settings);

                if (!settings)
                    return;

                if (EditorApplication.isPlayingOrWillChangePlaymode && !settings.UseHotKeyInPlayMode)
                {
                    return;
                }
            }
            else if (EditorApplication.isPlayingOrWillChangePlaymode && !instance.CurrentSettings.UseHotKeyInPlayMode)
            {
                return;
            }
        ...
        private static void FetchSettings(out GrabbitSettings settings)
        {
            var ids = AssetDatabase.FindAssets("t:GrabbitSettings");
            if (ids.Length == 0)
            {
            ...
            settings = AssetDatabase.LoadAssetAtPath<GrabbitSettings>(AssetDatabase.GUIDToAssetPath(ids[0]));
        }
        ...
        private static void StartGrabbit(GrabbitMode mode = GrabbitMode.PLACE, bool ignoreChangeOfTool = false)
        {
            if (Instance.CurrentTool)
            {
                VersionIndependentActivateTool();
            }
            else
            {
                FindOrCreateTool();
        ...
        private static void VersionIndependentActivateTool()
        {
#if UNITY_2020_2_OR_NEWER
            ToolManager.SetActiveTool(instance.CurrentTool);
#else
                EditorTools.SetActiveTool(instance.CurrentTool);
                instance.CurrentTool.Activate();
#endif
        }
        ...
        private static void FindOrCreateTool()
        {
            ...
            var grab = FindFirstObjectByType<GrabbitTool>();
#else
        ...
        {
            if (CurrentTool && CurrentTool.Active)
