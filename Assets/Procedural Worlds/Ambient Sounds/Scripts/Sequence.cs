...
#pragma warning restore
        ...
        private ClipData[] m_currentClips = new ClipData[0];
        /// <summary> Clips list based on Modifiers </summary>
        public ClipData[] Clips { get { return m_currentClips; } }
        ...
        public void UpdateModifiers() {
                ...
                if (m_modifiers[m].m_modClips) {
                    if (m_modifiers[m].FadeValue >= 0.5f) { //can only change the array fully or not at all ... so do it at half way up
                        if (m_modifiers[m].m_modClipsType == ClipModType.Add) {
                            m_tmpClips.AddRange(m_modifiers[m].m_clipData);
                        } else if (m_modifiers[m].m_modClipsType == ClipModType.Remove) {
                            for(int c = 0; c < m_modifiers[m].m_clipData.Length; ++c)
                            {
                                AudioClip clip = m_modifiers[m].m_clipData[c].m_clip;
                                m_tmpClips.RemoveAll(clp => clp.m_clip == clip);
                            }
                        } else { //Replace
                            m_tmpClips.Clear();
                            m_tmpClips.AddRange(m_modifiers[m].m_clipData);
            ...
            float newLength = 0f;
            m_tmpClips.RemoveAll(c => c.m_clip == null);
            for (int c = 0; c < m_tmpClips.Count; ++c)
            {
                AudioClip clp = m_tmpClips[c].m_clip;
                if (clp == null)
                    m_tmpClips.RemoveAt(c--);
                else
                    newLength += (AmbienceManager.GetAudioData(clp).Length - m_crossFade) * PlaybackSpeed;
            }
            ...
            {
                if (m_currentClips.Length != m_tmpClips.Count)
                    Array.Resize(ref m_currentClips, m_tmpClips.Count);
                for (int c = 0; c < m_currentClips.Length; ++c)
                    m_currentClips[c] = m_tmpClips[c];
            }
        }
